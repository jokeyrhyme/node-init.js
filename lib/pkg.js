...
async function getPkg(cwd /* : string */) /* : Promise<PackageJSON> */ {
  try {
    return await loadJson(path.join(cwd, 'package.json'));
  } catch (err) {
    return { name: '' }; // no file? default to new Object
  }
}
